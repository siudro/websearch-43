...
def respond(api, prompt):
    ...
    response = client.chat.completions.create(model = "gpt-4o-mini",
        messages = [
            {"role":"system","content": "you are a search web assistant "}
            ,
...
st.title("السلام عليكم انا اينشتاين")
st.subheader("انا اعرف كل شي بالعلوم")
user = st.text_input ("اسألني")
if user:
    result = web(user,api_web)
    if result:
        prompt = f"the question '{user}' answer the questionو Based on these search results '{result}'"
